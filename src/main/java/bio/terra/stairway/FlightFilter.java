...
/**
 * A FlightFilter is used to filter the flights on a flight enumeration.
 * You can build a filter with a fluent style:
 * <pre>
 * {@code
 ...
 * the value of {@code yesterday}) run by user ddtest (assuming you'd passed the email as an
 * input parameter.
 */
public class FlightFilter {
    ...
    /**
     * Filter by an input parameter. This is processed by converting the {@code value} into JSON
     * and doing a string comparison against the input parameter stored in the database. The {@code value} object
     * must be <b>exactly</b> the same class as the input parameter.
     ...
     */
    public FlightFilter addFilterInputParameter(String key, FlightFilterOp op, Object value) throws FlightFilterException {
        if (key == null) {
            throw new FlightFilterException("Key must be specified in an input filter");
        }
        FlightFilterPredicate predicate = new FlightFilterPredicate(key, op, value, STRING, makeParameterName());
     ...
     */
    void storePredicateValues(NamedParameterPreparedStatement statement) throws FlightFilterException {
        try {
    ...
    /**
     * Generate the enumeration query for flights. The list may be restricted by providing one
    ...
    /**
     * Generate the filter for the flight table. The right hand side
     * of any comparison op is generated as a named parameter.
     */
    ...
    /**
     * Generate the filter for the flight input table for Form 3: the case where there is more
     * that one predicate.
     */
