...
public interface QueueInterface {
   ...
   * @param processFunction Function to call for each message. The function returns true if
   *                        the message is successfully handled; false if it was not and should
   *                        remain on the queue.
   ...
   * NOTE: This method is intended for controlled test environments where a
   * cloud platform queue is reused. It should clear all messages from the queue, simply
   * ignoring them, so that tests can start from an empty queue.
   */
  void purgeQueue();
}
